...
if(isset($_POST['submit'])){
    ...
    if($getdata){
      // echo "<script>alert('EMPLOYEE ALREADY EXIST');</script>";
        ...
        $insertquery = mysqli_query($customercon, "INSERT INTO employeeaccount VALUES(null,'$firstname','$lastname','$middlename','$contact','$email','$bday','$date','100')");
       if($insertquery){
        $mysql = mysqli_query($customercon,"SELECT * FROM employeeaccount ORDER BY emp_id DESC");
        $getinfo = mysqli_fetch_object($mysql);

        $customercredential = $getinfo -> emp_id;
/// add ng account para sakanila

if($type =="ADMIN"){
  if($customercredential < 10){
          $username = "ADM-00".$customercredential;
        }elseif($customercredential >=10){
          $username = "ADM-0".$customercredential;
        }elseif($customercredential >=100){
          $username = "ADM-".$customercredential;
        }
}else{
    if($customercredential < 10){
          $username = "UWS-00".$customercredential;
        }elseif($customercredential >=10){
          $username = "UWS-0".$customercredential;

        }elseif($customercredential >=100){
          $username = "UWS-".$customercredential;
        }
}
        
$dayb = substr($bday, 0,4);
$pass = $dayb.$lastname;

        $insertinaccount = mysqli_query($customercon,"INSERT INTO accounts VALUES(null,'$username','$pass','$type')");
        if($insertinaccount){
          ...
          $update = mysqli_query($customercon,"UPDATE employeeaccount SET accountid='$accountid' WHERE emp_id ='$customercredential'");
             if($update){
...
<head>
    ...
    <title>UWS-Member-Add-Employee</title>
...
<div class="row">
    <div class="col" style="display: flex; align-items:center; color: darkslategray;">
      <i class="bi bi-person-rolodex img-fluid fs-1 me-3" ></i> MEMBER / ADD EMPLOYEE
...
<div class="container-fluid mb-0">
         <div class="row mb-1"style="background-color:#007c92;" >
                        <div class="col-sm-2">
...
<form method="POST" class="modal-content bg-light p-4 mt-2" style="border-radius: 1rem;">
        ...
        <div class="row">
            <div class="col text-wrap mt-4" id="addcol1">
                    ...
                    <input type="number" name="contact" id="" class="form-control mb-4" value="<?=$contact?>" >
                     
                  
            </div>
            <div class="col mt-4">
                 <label for="">Email <i></i></label>
                    <input type="email" name="email" id="" class="form-control mb-4" value="" required >
                        <label for="">Birthdate<i style="color: red;">*</i></label>
                    <input type="date" name="bday" id="" class="form-control mb-4">
